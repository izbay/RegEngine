...
(defonce ^:dynamic *log-physics-events* false)
...
TODO: Perhaps send a msg to the first player?  We can't use the clj-minecraft {:msg} automessaging; not all events have an associated player."
  ...
  (println (format "Event '%s' occured." (.getEventName ev))))


(defn- block-physics-handler [^BlockEvent ev]
  "Feedback handler.
Now also cancels events if *cancel-event* is true."
  ...
  (let [block (.getBlock ev)
    ...
    (when *log-physics-events*
      (def *physics-event-log* (cons ev *physics-event-log*))))
  (when *cancel-event*
...
(defn event-cancelling-handler [^Event ev]
  (when *cancel-event*
